...
AdbP = 'Botit\\BotitCore\\adb.exe'
EmuP = 'Botit\\BotitCore\\BotitHedless.exe'
emulatorEX = 'Botit\\BotitCore\\emulator\\emulator.exe'
HaxmCheck = 'Botit\\BotitCore\\HardwareAcceleratedManager\\haxm_check.exe'
HaxmInstaller = 'Botit\\BotitCore\\HardwareAcceleratedManager\\silent_install.bat'
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.imgfolderG1.clicked.connect(self.imgfolderGD1)
        self.imgfolderG2.clicked.connect(self.imgfolderGD2)
        self.imgfolderG3.clicked.connect(self.imgfolderGD3)
        self.imgfolderG4.clicked.connect(self.imgfolderGD4)
        self.imgfolderG5.clicked.connect(self.imgfolderGD5)
        self.imgfolderG6.clicked.connect(self.imgfolderGD6)
        self.imgfolderG7.clicked.connect(self.imgfolderGD7)

        import requests, sys
        print("Connecting To GitHub, Checking INI")
        try:
            link = 'https://raw.githubusercontent.com/DizzyduckAR/BotIt/master/TXT/UpdateBots.ini'
            file_name = "Botit\\BotitCore\\UpdateBots.ini"
            ...
            print("Something went wrong in GitHub INI Grab")
...
#
         ...
         #                      + 'Updater.exe'
          #      print(Startg1shell)
         #       subprocess.Popen(Startg1shell, stdout=PIPE)
        #        sys.exit()
       #         print("Updater Ready")
        try:
            link = 'https://raw.githubusercontent.com/DizzyduckAR/BotIt/master/TXT/BotitNews.ini'
            file_name = "Botit\\BotitCore\\BotitNews.ini"
            with open(file_name, "wb") as f:
                ...
                if total_length is None:  # no content length header
                    ...
                    for data in response.iter_content(chunk_size=4096):
                       ...
                       # write1 = write("\r[%s%s]" % ('=' * done, ' ' * (50 - done)
    ...
    def EmuuiTab(self):
        ...
        try:
            ...
            Emuchk = os.path.isfile('Botit/BotitCore/emulator/emulator.exe')
            if Emuchk:
                ...
                if ret == QMessageBox.Yes:
                    ...
                    with open(file_name, "wb") as f:
                        ...
                        if total_length is None:  # no content length header
                            ...
                            for data in response.iter_content(chunk_size=4096):
                              ...
                              #  self.SetStatusBar.showMessage(' was clicked')
                    ...
                    handle.extractall('Botit/BotitCore/')
                    print("Emulator Ready")

            EmuIMGchk = os.path.isfile('Botit/BotitCore/system-images/9/x86/build.prop')
            if EmuIMGchk:
                ...
                if ret == QMessageBox.Yes:
                    ...
                    handle = zipfile.ZipFile(Targetzip1)
                    handle.extractall('Botit/BotitCore//system-images/9')
                    print("System Image Ready")
        except:
            print("INI Read Error Detected")
    ...
    def BotitTab(self):
        ...
        try:
            g1exechack = str('Botit\\BotitCore\\Bots\\Pokemon Masters\\') \
                         + 'Pokemon Masters.exe'

            if os.path.exists(g1exechack):
                self.Startg1.setEnabled(True)

            g1exechack = str('Botit\\BotitCore\\\\Bots\\DigimonReA\\') \
                         + 'DigimonReA.exe'

            if os.path.exists(g1exechack):
                self.Startg2.setEnabled(True)

            g1exechack = str('Botit\\BotitCore\\Bots\\Summoners War\\') \
                         + 'Summoners War.exe'

            if os.path.exists(g1exechack):
                self.Startg3.setEnabled(True)

            g1exechack = str('Botit\\BotitCore\\Bots\\Raid\\') \
                         + 'Raid.exe'

            if os.path.exists(g1exechack):
                self.Startg5.setEnabled(True)

            g1exechack = str('Botit\\BotitCore\\Bots\\Calibria\\') \
                         + 'Calibria.exe'

            if os.path.exists(g1exechack):
                self.Startg4.setEnabled(True)

            g1exechack = str('Botit\\BotitCore\\Bots\\Unison League\\') \
                         + 'Unison League.exe'

            if os.path.exists(g1exechack):
                self.Startg6.setEnabled(True)

            g1exechack = str('Botit\\BotitCor\\\Bots\\The Seven Deadly Sins\\') \
                         + '7Deadly.exe'
            ...
            Remover = pathlib.Path().absolute()
            location = str(Remover) \
                       + '\\Botit\\BotitCore\\Bots\\Pokemon Masters\\'
            files_in_dir = []
            g1localV = "0.0.0"
            for r, d, f in os.walk(location):
                for item in f:
            ...
            Remover = pathlib.Path().absolute()
            location = str(Remover) \
                       + '\\Botit\\BotitCore\\Bots\\DigimonReA\\'
            files_in_dir = []
            g2localV = "0.0.0"
            for r, d, f in os.walk(location):
                for item in f:
            ...
            Remover = pathlib.Path().absolute()
            location = str(Remover) \
                       + '\\Botit\\BotitCore\\Bots\\Summoners War\\'
            files_in_dir = []
            g3localV = "0.0.0"
            for r, d, f in os.walk(location):
                for item in f:
            ...
            Remover = pathlib.Path().absolute()
            location = str(Remover) \
                       + '\\Botit\\BotitCore\\Bots\\Unison League\\'
            files_in_dir = []
            g6localV = "0.0.0"
            for r, d, f in os.walk(location):
                for item in f:
            ...
            Remover = pathlib.Path().absolute()
            location = str(Remover) \
                       + '\\Botit\\BotitCore\\Bots\\The Seven Deadly Sins\\'
            files_in_dir = []
            g7localV = "0.0.0"
            for r, d, f in os.walk(location):
                for item in f:
    ...
    def imgfolderGD1(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\Pokemon Masters\\img'
        try:
    ...
    def Startg1F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\Pokemon Masters\\Pokemon Masters.exe'
    ...
    def Startg2F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\\DigimonReA\\DigimonReA.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD2(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\DigimonReA\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Startg3F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\\Summoners War\\Summoners War.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD3(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\Summoners War\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Startg4F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + '\\Botit\\BotitCore\\Bots\Calibria\\Calibria.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD4(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\Calibria\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Startg5F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\Raid\\Raid.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD5(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\Raid\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Startg6F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\\Unison League\\Unison League.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD6(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\Unison League\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Startg7F(self):
        ...
        Startg1shell = str(Remover) + '\\' \
                       + 'Botit\\BotitCore\\Bots\\The Seven Deadly Sins\\7Deadly.exe'

        runnow = po(Startg1shell, stdout=PIPE, shell=True)

    def imgfolderGD7(self):

        Remover = pathlib.Path().absolute()
        location = str(Remover) \
                   + '\\Botit\\BotitCore\\Bots\\The Seven Deadly Sins\\img'
        try:
            os.startfile(location)
        except:
            print("No folder")
    ...
    def Downloadg1F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\Pokemon Masters')
    ...
    def Downloadg2F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\DigimonReA')
        print("Extracting Bot")
    ...
    def Downloadg3F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\Summoners War')
        print("Extracting Bot")
    ...
    def Downloadg4F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\Calibria')
        print("Extracting Bot")
    ...
    def Downloadg5F(self):
        ...
        Targetzip1 = 'Raid.zip'
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\Raid')
        print("Extracting Bot")
    ...
    def Downloadg6F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\\Bots\\Unison League')
        print("Extracting Bot")
    ...
    def Downloadg7F(self):
        ...
        handle = zipfile.ZipFile(Targetzip1)
        handle.extractall('Botit\\BotitCore\Bots\The Seven Deadly Sins')
        print("Extracting Bot")
        self.Downloadg6.setEnabled(False)
        ...
        Remover = pathlib.Path().absolute()
        g1exechack = str('Botit\\BotitCore\\Bots\\The Seven Deadly Sins\\') \
                     + '7Deadly.exe'
        print(g1exechack)
        if os.path.exists(g1exechack):
            self.Startg7.setEnabled(True)
    ...
    def EmuboxlistEdit(self, accion):
        try:
            ...
            print("Error Detected. Box Setting INI Damaged")
    ...
    def CreateEEmulatorP(self, accion):
        ...
        if os.path.exists(file_path):
            ...
            file_copy1A = str(Path3) \
                          + '\\Botit\\BotitCore\\avd\\DemoINI\\Config.ini'
            ...
            file_copy3A = str(Path3) \
                          + '\\Botit\\BotitCore\\avd\\DemoINI\\Demo.ini'
            ...
            if self.IntelCheck.isChecked():
                p = subprocess.Popen(HaxmCheck, stdout=subprocess.PIPE)

                for line in p.stdout:
                    lineSet = (format(line.rstrip().decode("utf-8")))
                    print(lineSet)
                    if "VT support -- yes" in lineSet:
                        print("VT support Check OK")

                    elif "NX support -- yes" in lineSet:
                        print("NX support Check OK")

                    elif "VT support -- no" in lineSet:
                        print("VT support Check Failed")
                        ret = QMessageBox.question(self, 'Error Detected',
                                                   "VT support Missing. allow Intel Haxm Unlock",
                                                   QMessageBox.Yes | QMessageBox.No)
                        if ret == QMessageBox.Yes:
                            p = subprocess.Popen(HaxmInstaller, stdout=subprocess.PIPE)
                            for line in p.stdout:
                                lineSet = (format(line.rstrip().decode("utf-8")))
                                print(lineSet)
                                return
                    ...
                    elif "NX support -- no" in lineSet:
                        print("NX support Check Failed")
                        ret = QMessageBox.question(self, 'Error Detected',
                                                   "NX support Missing. allow Intel Haxm Unlock",
                                                   QMessageBox.Yes | QMessageBox.No)
                        if ret == QMessageBox.Yes:
                            p = subprocess.Popen(HaxmInstaller, stdout=subprocess.PIPE)
                            for line in p.stdout:
    ...
    def DeleteEEmulatorP(self, accion):
        ...
        Emudeletefile = str('Botit\\BotitCore\\avd\\') \
                        + EmuBoxit \
                        + '.ini'
        print(Emudeletefile)
        ...
        EmudeleteFolder = str('Botit\\BotitCore\\avd\\') \
                          + EmuBoxit \
                          + '.avd\\'
